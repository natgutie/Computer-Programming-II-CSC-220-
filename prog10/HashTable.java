...
public class HashTable<K, V> extends AbstractMap<K, V> {
  ...
  public V put (K key, V value) {
    ...
    int indexEntry = hashIndex(key);
    Entry newEntry = new Entry(key, value);
    newEntry.next = table[indexEntry];
    table[indexEntry] = newEntry;
    // Increment size.
    size++;
  ...
  private void rehash (int newCapacity) {
    ...
    for (int i = 0; i < oldTable.length; i++) {
      ...
      while(oldTable[i] != null){
        ...
        int index = hashIndex(nextEntry.key);
        nextEntry.next = table[index];
  ...
  public V remove (Object key) {
    ...
    for(Entry previousEntry = table[keyIndex]; previousEntry.next != null; previousEntry = previousEntry.next){

      if(previousEntry.next.key.equals(key)){
        V value = previousEntry.next.value;
