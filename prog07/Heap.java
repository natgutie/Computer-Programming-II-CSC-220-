...
public class Heap <E> extends AbstractQueue<E> {
  ...
  */
  public boolean offer (E element) {
    ...
    while(compare(index, parent(index)) < 0){ // child is less than parent
      index = swap(index, parent(index));//swap
    }
    ///
  ...
  */
  public E poll() {
    ...
    // EXERCISE
    // If the heap contains only one element, then remove it and return it.
    if(size() == 1) {
      // heap is represented as theElements (theElements.remove)
    ...
    int index = 0;
    // EXERCISE
    // swap root with last offer and remove the last
    // while (have children && at least one child is less than me)
    // While the element at index is greater than one of its children...
    // (Use isValid, left, right, and compare.)
    while((isValid(left(index)) && compare(index, left(index)) > 0) || (isValid(right(index)) && compare(index, right(index)) > 0)) { // check if you have a left child something like isValid(L(i)) && compare(i, L(i))
      ...
      }else{ // Else swap with left child.
  ...
  public boolean remove (Object o) {
    ...
    while ((isValid(left(index)) && compare(index, left(index)) > 0) || (isValid(right(index)) && compare(index, right(index)) > 0)) { // check if you have a left child something like isValid(L(i)) && compare(i, L(i))
      ...
      //switch last thing with the thing you want to remove and remove the last thing (poll)
      // check if parents are valid (need to be bigger than parent if not swap)
