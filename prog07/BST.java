...
public class BST <K extends Comparable<K>, V>
  ...
  private Entry find (K key, Entry root) {
    // EXERCISE:
    // recursion --> base case: found it return it, null return
    // recursive step: if you are bigger than (find on the right) or if smaller (find on the left)
  ...
  public V get (Object key) {
    // EXERCISE:
    // not allowed to use containsKey and only run find once
  ...
  public V put (K key, V value) {
    // EXERCISE:
    // going to return a tree with a new node added, update your root based of what you get back from add
    Entry entryFound = find(key, root);

    if(entryFound == null) {
      root = add(key, value, root);
      return null;
    }else{
      V old = entryFound.setValue(value);
      return old;
    }
  ...
  public V remove (Object keyAsObject) {
    K key = (K) keyAsObject;
    //Implement the public remove method using find and the private remove method.
    Entry removeNode = find(key, root);
  ...
  private Entry remove (K key, Entry root) {
    // Implement the private remove method based on the notes.  It wil call removeRoot.
  ...
  private Entry removeRoot (Entry root) {
    ...
    // Implement removeRoot and the other methods it needs.  The correct output is in bst-remove.txt.
    ...
    Entry newRootValue = getMaximum(root.left);
    root.left = removeMaximum(root.left);
    newRootValue.left = root.left;
    newRootValue.right = root.right;
    return newRootValue;
  }
